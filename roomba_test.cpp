...
int send_song_command(int song, int port_in)
{
	...
	byte=set_christmas_song_command(sbuf, song);//songをセット
...
void jstest()
{
	...
	if (joyGetPosEx(JOYSTICKID1, &joyInfoEx) == JOYERR_NOERROR)
	{
		...
		/* XY軸の最大値、最小値、現在値の表示 */
		/*sprintf(buf, "(Xmin, Xmax) = (%u, %u)", joyCaps.wXmin, joyCaps.wXmax);
		printf(buf);
		sprintf(buf, "(Ymin, Ymax) = (%u, %u)", joyCaps.wYmin, joyCaps.wYmax);
		printf(buf);
		sprintf(buf, "(Xpos, Ypos) = (%u, %u)\n", joyInfoEx.dwXpos, joyInfoEx.dwYpos);
		printf(buf);*/
		sprintf(buf,"X:%u/%u, Y:%u/%u\n",joyInfoEx.dwXpos,joyCaps.wXmax,joyInfoEx.dwYpos,joyCaps.wYmax);
		printf(buf);
		sprintf(buf,"Z:%u/%u, R:%u/%u\n",joyInfoEx.dwZpos,joyCaps.wZmax,joyInfoEx.dwRpos,joyCaps.wRmax);
		printf(buf);

		/* ボタンの総数 */
		sprintf(buf, "NumButton = %u\n", joyCaps.wNumButtons);
		//TextOut(hDC, 0, 80, buf, lstrlen(buf));
		printf(buf);

		/* 押下中のボタンを取得。32ボタンまで取得可能 */
		sprintf(buf, "Push Buttons : ");
        ...
        {
            strcat(buf, "02 ");
            serial *s=&rb_serial[0];
            char sbuf[9];//音の数×2＋3用意する
            int byte=9;

            sbuf[0]=140;//song command
            sbuf[1]=2;//song number
            sbuf[2]=3;//音の数
            sbuf[3]=48;//音1
            sbuf[4]=10;//音1の長さ
            sbuf[5]=50;//音2の長さ
            ...
            s->send(sbuf,byte);//Song登録コマンド送信


        }
		if (joyInfoEx.dwButtons & JOY_BUTTON3)
        {
            strcat(buf, "03 ");
            serial *s=&rb_serial[0];
            char sbuf[4];
            ...
            sbuf[0]=RB_LEDS;
            sbuf[1]=0xf;//光のパターン
            sbuf[2]=0;//color 0=green 255=red
            sbuf[3]=255;//intensity(強さ)

            s->send(sbuf,byte);//LEDコマンド送信
        }
		if (joyInfoEx.dwButtons & JOY_BUTTON4)
        {
            strcat(buf, "04 ");
            serial *s=&rb_serial[0];
            char sbuf[4];
            ...
            sbuf[1]=0x0;//光のパターン
            sbuf[2]=0;//color 0=green 255=red
            sbuf[3]=0;//intensity(強さ)

            s->send(sbuf,byte);//LEDコマンド送信
        }


		if (joyInfoEx.dwButtons & JOY_BUTTON5)
        {
            strcat(buf, "05 ");
            serial *s=&rb_serial[0];
            char sbuf[2];
            ...
            sbuf[0]=(unsigned char)141;//play command
            sbuf[1]=(unsigned char)2;//song number

            s->send(sbuf,byte);//Song登録コマンド送信
        }

		if (joyInfoEx.dwButtons & JOY_BUTTON6)
        {
            strcat(buf, "06 ");
            //get_sensors()

        }
		if (joyInfoEx.dwButtons & JOY_BUTTON7) strcat(buf, "07 ");
		if (joyInfoEx.dwButtons & JOY_BUTTON8) strcat(buf, "08 ");

		if (joyInfoEx.dwButtons & JOY_BUTTON9) strcat(buf, "09 ");
		if (joyInfoEx.dwButtons & JOY_BUTTON10) strcat(buf, "10 ");
		if (joyInfoEx.dwButtons & JOY_BUTTON11) strcat(buf, "11 ");
		if (joyInfoEx.dwButtons & JOY_BUTTON12) strcat(buf, "12 ");
//TextOut(hDC, 0, 100, buf, lstrlen(buf));
	...
	{
		strcat(buf, "No Joystick - 情報を取得できません");
		//TextOut(hDC, 0, 00, buf, lstrlen(buf));
		printf(buf);

	}
	double lval=joyInfoEx.dwYpos*(510.0/65535.0)-255;
	double rval=joyInfoEx.dwRpos*(510.0/65535.0)-255;
	double v=0.5;//最大速度に対する倍率
	lval*=v;
    rval*=v;
    printf("(lval,rval)=(%f,%f)\n",lval,rval);

	if(fabs(lval)<15)lval=0;
	if(fabs(rval)<15)rval=0;

	send_drive_command((int)lval, (int)rval, port);
...
void print_sensors(int port_in)
{
    ...
    \n",
       rss->stat,
       rss->EncL,rss->EncR,
       rss->LBumper, rss->LBumper_L,rss->LBumper_FL,rss->LBumper_CL,rss->LBumper_CR,
       rss->LBumper_FR,rss->LBumper_R,
        rss->Angle,
        rss->Distance,
        rss->TimeNow
       );
...
//--------------------------
//-------------------------------------
void print_sensors2(int port_in)
{
    //RoombaSensor *s1=&sensor1;
	RoombaSensor *rss=&roomba[port_in].sensor;
    ...
    {
        //ポート準備ができていない場合
        printf("print_sensors2() serial port is not ready.\n");
        return;
    }

    printf("LBumper_FL=%d,LBumper_CL=%d,LBumper_CR=%d,\LBumper_FR=%d,n",
           rss->LBumper_FL,rss->LBumper_CL,rss->LBumper_CR,rss->LBumper_FR);

    if(rss->LBumper_FL>100)
    {
        printf("FL\n");
        char buf[128];
        strcat(buf, "---");
